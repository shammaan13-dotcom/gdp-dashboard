...
def convert_macl_pdf_to_xlsx(pdf_bytes):
    ...
    with pdfplumber.open(pdf_stream) as pdf:
        ...
        ordered_weekdays = sorted(weekday_page_indexes.items(), key=lambda item: item[1])
        for pos, (short_day, start_page) in enumerate(ordered_weekdays):
            next_weekday_page = (ordered_weekdays[pos + 1][1]
                                 if pos + 1 < len(ordered_weekdays) else None)

            # The Sunday section ends when DAYS OF OPS starts.  For all other
            # weekdays it ends immediately before the next weekday heading.
            if next_weekday_page is not None:
                end_page = next_weekday_page
            else:
                end_page = len(pdf.pages)
                for probe in range(start_page + 1, len(pdf.pages)):
                    probe_txt = (pdf.pages[probe].extract_text() or "").upper()
                    if "DAYS OF OPS" in probe_txt and ("SUMMER" in probe_txt or "WINTER" in probe_txt):
                        end_page = probe
                        break
    ...
    audit.column_dimensions["B"].width = 55
