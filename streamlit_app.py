...
def convert_macl_pdf_to_xlsx(pdf_bytes):
    ...
    with pdfplumber.open(pdf_stream) as pdf:
        ...
        # Hard completeness validation.  Re-scan every owned physical page and
        # compare its table-row count with what was carried into the workbook.
        # If a continuation page is ever missed, conversion STOPS rather than
        # producing a misleading partial Excel file.
        for short_day in ["MON","TUE","WED","THU","FRI","SAT","SUN"]:
            ...
            for page_index in owned:
                expected += len(_extract_pdf_schedule_rows(pdf.pages[page_index], fitz_doc[page_index]))
            actual = len(weekday_rows[short_day])
            if actual != expected:
                raise ValueError(
                    f"MACL PDF conversion stopped: {short_day} incomplete. "
                    f"PDF contains {expected} schedule rows across pages "
                    f"{', '.join(str(i+1) for i in owned)}, but only {actual} were captured."
                )
