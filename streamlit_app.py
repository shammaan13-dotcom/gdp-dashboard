...
def convert_macl_pdf_to_xlsx(pdf_bytes):
    ...
    with pdfplumber.open(pdf_stream) as pdf:
        ...
        # V28: DYNAMIC PHYSICAL-PAGE SECTION READER.
        # A weekday begins on the page carrying its weekday heading. Every
        # following physical page belongs to that weekday until the next weekday
        # heading begins. No fixed page numbers, no fixed total page count and no
        # fixed expected row counts are used.
        #
        # First locate the true DAYS OF OPS section title. Do not confuse the
        # "DAYS OF OPS" table-column header on normal weekday pages with the
        # actual DAYS OF OPS section.
        days_start = None
        for page_index, page in enumerate(pdf.pages):
            txt = (page.extract_text() or "").upper()
            if re.search(r"\b(?:SUMMER|WINTER)\s+\d{4}(?:-\d{2,4})?\s+DAYS OF OPS\b", txt):
        ...
        heading_pages = [(weekday_page_indexes[d], d) for d in ordered_days]
        heading_pages.sort()

        # Sanity-check weekday order and ensure all weekday sections occur before
        # the DAYS OF OPS section.
        if [d for _, d in heading_pages] != ordered_days:
            raise ValueError("MACL PDF conversion stopped: weekday headings are not in MON-SUN order.")
        if heading_pages[-1][0] >= days_start:
            raise ValueError("MACL PDF conversion stopped: weekday section overlaps DAYS OF OPS.")

        page_counts = {d: [] for d in ordered_days}
        source_line_counts = {}

        for pos, (section_start, short_day) in enumerate(heading_pages):
            section_end = heading_pages[pos + 1][0] if pos + 1 < len(heading_pages) else days_start
            owned_pages = list(range(section_start, section_end))
            if not owned_pages:
                raise ValueError(f"MACL PDF conversion stopped: no PDF pages assigned to {short_day}.")

            expected = 0
            for page_index in owned_pages:
                # Independent source count: every MACL movement line carries a
                # seven-digit DAYS OF OPS mask.
                ...
                extracted = _extract_pdf_schedule_rows(
                ...
                summary["red_rows"] += sum(
                    1 for _, fill in extracted if fill == "FF0000"
                )
            ...
            if actual != expected:
                raise ValueError(
                    f"MACL PDF conversion stopped: {short_day} incomplete. "
                    f"PDF source rows={expected}; converted rows={actual}; "
                    f"physical pages={','.join(str(i+1) for i in owned_pages)}. "
...
if macl_file is not None:
    ...
    if macl_name.endswith(".pdf"):
        ...
        try:
            ...
            counts = pdf_conversion_summary.get("weekday_counts", {})
            count_text = " | ".join(
                f"{d} {counts.get(d, 0)}"
                for d in ["MON","TUE","WED","THU","FRI","SAT","SUN"]
            )
            st.success(
                f"V28 PDF converter — MACL PDF converted successfully — "
